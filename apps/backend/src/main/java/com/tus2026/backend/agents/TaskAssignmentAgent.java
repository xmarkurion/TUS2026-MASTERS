...
public class TaskAssignmentAgent {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            2. For each task provided in the message, choose the best member based on:
               ...
               - Member capacity must be >= task effort (hours) — skip the task if no one qualifies
        ...
        try {
            ...
            while (!(batch = taskRepository.findByAssigneeIdIsNull()
                ...
                if (reasons.isEmpty()) {
                    emitter.send(SseEmitter.event()
                            .name("warning")
                            .data("Turn " + turnNumber + ": agent made no assignments. Stopping."));
                    break;
                }
                ...
                List<AssignmentResult> batchResults = reasons.entrySet().stream()
                        .map(entry -> {
                            ...
                            return taskRepository.findById(taskId)
                                    ...
                                    .flatMap(t -> memberRepository.findById(t.getAssigneeId())
                                            .map(m -> new AssignmentResult(
                                                    t.getId(), t.getTaskName(),
                                                    m.getId(), m.getName(),
                                                    reason)))
                                    .orElse(null);
                        })
    ...
    public AssignmentResult assignSingleTask(String taskId) {
        ...
        if (!anyoneAvailable) {
            throw new IllegalStateException(
                    "No suitable people found: no member has sufficient capacity for this task (" + minEffort + "h).");
        }
    ...
    private String buildUserMessage(List<Task> tasks) {
        String taskList = tasks.stream()
                .map(t -> String.format("  - id=%s | name=%s | difficulty=%s | effort=%dh | desc=%s",
                        t.getId(), t.getTaskName(), t.getDifficulty(), t.getEffort(), t.getTaskDesc()))
